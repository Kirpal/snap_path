...
class MapboxRepository implements MapRepository {
  ...
  Future<ElevationData> getElevation(List<LatLng> path) async {
    if (path.length > 0) {
      ...
      var allTiles =
          tileCoords.fold<Set<Point<int>>>(Set<Point<int>>(), (tileSet, tile) {
    ...
    if ((query?.isNotEmpty ?? false) && query.length <= 256) {
      ...
      if (response.statusCode == 200) {
        ...
        for (var feature in features) {
          ...
          var place = placeContext.firstWhere(
              (m) => m['id'].startsWith('place.'),
              orElse: () => {})['text'];
          var region = placeContext.firstWhere(
              (m) => m['id'].startsWith('region.'),
              orElse: () => {})['text'];
          var country = placeContext.firstWhere(
          ...
          var center = LatLng((feature['center'][1] as num).toDouble(),
              (feature['center'][0] as num).toDouble());
          LatLngBounds bounds = LatLngBounds(center);
          if (feature['bbox'] != null) {
            var topLeft = LatLng((feature['bbox'][1] as num).toDouble(),
                (feature['bbox'][0] as num).toDouble());
            var bottomRight = LatLng((feature['bbox'][3] as num).toDouble(),
