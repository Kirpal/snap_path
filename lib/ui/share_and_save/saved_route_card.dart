  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
        boxShadow: [
          BoxShadow(blurRadius: 10, offset: Offset(0, 10), color: Colors.black.withOpacity(0.02))
        ],
      ),
      child: Stack(
        children: [
                  ...
                  child: ClipRRect(
                    ...
                    child: LayoutBuilder(
                      builder: (context, constraints) {
                        var width = (constraints.maxWidth * MediaQuery.of(context).devicePixelRatio).round();
                        var height = (constraints.maxHeight * MediaQuery.of(context).devicePixelRatio).round();
                        return Image.network(context.watch<MapRepository>().getPreview(route.coordinates, width, height));
                      },
                    ),
                  ),
                ),
                  ...
                  child: Row(
                    children: <Widget>[
                      HeaderFigure(
                        ...
                        data: (metric ? route.distance.inKilometers : route.distance.inMiles).toStringAsFixed(2),
                        unit: metric ? 'kilometers' : 'miles',
                      ),
                      HeaderFigure(
                        group: headerTextGroup,
                        data: (metric ? route.elevationGain.inMeters : route.elevationGain.inFeet).round().toString(),
            ...
            child: InkWell(
              ...
              onTap: () {
                ...
                context.read<AppState>().moveToShow(
                  ...
                  padding: EdgeInsets.all(24) + EdgeInsets.only(bottom: 120) + MediaQuery.of(context).viewPadding,
                );
          ...
          Positioned(
            ...
            child: PopupMenuButton<bool>(
              ...
              itemBuilder: (context) => [
                PopupMenuItem(child: Text('Delete'), value: true,)
              ],
              onSelected: (delete) {
                if (delete) {
                  context.read<AppState>().deleteRoute(index);
                }
              },
            ),
          ),
